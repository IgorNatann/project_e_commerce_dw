#!/usr/bin/env python3
"""Smoke test recorrente dos filtros do dashboard de metas."""
...
DEFAULT_APP_PATH = Path("/app/dashboards/streamlit/metas/app.py")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Valida filtros do dashboard de metas com a mesma consulta usada no app."
    )
    parser.add_argument(
        "--app-path",
        default=str(DEFAULT_APP_PATH),
        help=f"Caminho para o app Streamlit de metas (default: {DEFAULT_APP_PATH}).",
    )
    parser.add_argument(
        "--json",
        action="store_true",
...
def _load_module(app_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("dash_metas_app", app_path)
...
def run_checks(app_module) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    load_goals_data = _unwrap_cache(app_module._load_goals_data)
    ...
    def load(
        *,
        ...
        return load_goals_data(
            conn_str=conn_str,
            start_date=s,
            end_date=e,
            regionais=tuple(regionais),
            equipes=tuple(equipes),
            vendedores=tuple(vendedores),
            tipos_equipe=tuple(tipos_equipe),
        )
    ...
    check_filter("filtro_regional", "regionais", "regionais", "regional")
    check_filter("filtro_tipo_equipe", "tipos_equipe", "tipos_equipe", "tipo_equipe")
    ...
    combo_df = load(
        s=sample_date,
        e=sample_date,
        regionais=(str(sample_row["regional"]),),
        tipos_equipe=(str(sample_row["tipo_equipe"]),),
        equipes=(str(sample_row["nome_equipe"]),),
        vendedores=(str(sample_row["nome_vendedor"]),),
    )
    tests.append(
        {
            ...
            "sample": {
                ...
                "regional": str(sample_row["regional"]),
                "tipo_equipe": str(sample_row["tipo_equipe"]),
    ...
    missing_df = load(regionais=("__REGIONAL_INEXISTENTE__",))
    tests.append(
        {
    ...
    summary = {
        ...
        "metadata_counts": {
            "regionais": len(metadata.get("regionais", [])),
            "tipos_equipe": len(metadata.get("tipos_equipe", [])),
